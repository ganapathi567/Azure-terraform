...
locals {
  ...
  storage_container_name = join("-", [local.environment, local.tesco_team_name, "mongo-config"])
  storage_container_backup_name = join("-", [local.environment, local.tesco_team_name, "mongo-config-backup"])
  resource_name          = join("-", [local.resource_base_name, "mongo-config"])
  ...
  replica_set_name       = join("-", [local.tesco_team_name, "mongo-config"])
...
module "mongo_vmss" {
  ...
  storage_account_name       = data.azurerm_storage_account.mongo_storage_account.name
  #user_assigned_identity_ids = module.mongo_config_managed_identity.ids
...
#module "mongo_config_managed_identity" {
...
#module "mongo_config_managed_identity_role_assignment" {
#  source = "../mongo_modules/access/role-assignment"

#  assignee_principal_id  = module.mongo_config_managed_identity.principal_ids[0]
#  role_assignment_scopes = [local.storage_container_name]
#  role_definition_id     = data.azurerm_role_definition.blob_reader.id
#}

#module "mongo_config_managed_identity_role_assignment_backups" {
#  source = "../mongo_modules/access/role-assignment"
