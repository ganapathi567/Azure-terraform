...
#  template = "${file("./templates/vmss_test_private_ip.sh")}"
#}
...
#        "_id"    = "${i}"
#        host     = "${element(module.mongo_cmn_shard_nic.ip_addresses, i)}"
...
data "template_file" "mongo_installation" {
  ...
  vars     = {
    ...
    backup_scheduler_file_content         = module.mongo_cmn_shard_backup.scheduler_script_content
  }
}
