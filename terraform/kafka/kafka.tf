...
resource "azurerm_subnet" "sts_kafka_subnet" {
  ...
  resource_group_name  = data.azurerm_resource_group.sts_kafka_vnet_rg.name
  virtual_network_name = data.azurerm_virtual_network.sts_kafka_vnet.name
...
module "kafka_nic" {
  source = "./kafka_modules/compute/nic"

  resource_group_name           = azurerm_resource_group.kafka_resource_group.name
  location                      = var.location
  vm_count                      = var.kafka_broker_count
  vm_name                       = local.resource_name
  subnet_id                     = azurerm_subnet.sts_kafka_subnet.id
  private_ip_address_allocation = "Dynamic"
  #private_ip_address_allocation = "Static"
  #private_ip_addresses          = var.kafka_ip_addresses

  tags = var.tesco_tags
}

module "kafka_vm" {
  source = "./kafka_modules/compute/vm"

  resource_group_name = azurerm_resource_group.kafka_resource_group.name
  location            = var.location

  availability_zones = var.kafka_availability_zones
  vm_count           = var.kafka_broker_count
  vm_name            = local.resource_name
  vm_size            = var.kafka_vm_size
  nic_ids            = module.kafka_nic.ids
  ...
  module_depends_on = [azurerm_storage_container.kafka_storage_container]

  tags = var.tesco_tags
}

module "kafka_vm_ext" {
  source = "./kafka_modules/compute/vm-ext"

  vm_ids              = module.kafka_vm.ids
  script_file_content = {for i in range(length(module.kafka_vm.ids)) : element(module.kafka_vm.ids, i) => element(data.template_file.kafka_broker_installation.*.rendered, i)}

  module_depends_on = [module.kafka_disk_attachment]

  tags = var.tesco_tags
}


module "kafka_data_disk" {
  source = "./kafka_modules/storage/managed-disk"

  vm_count           = var.kafka_broker_count
  resource_group     = local.resource_name
  region             = var.location
  vm_name            = local.resource_name
  availability_zones = var.kafka_availability_zones
  disk_size_gb       = 250

  module_depends_on = [azurerm_resource_group.kafka_resource_group]

  tags = var.tesco_tags
}

module "kafka_disk_attachment" {
  source = "./kafka_modules/storage/data-disk-attachment"

  disk_ids = module.kafka_data_disk.ids
  vm_ids   = module.kafka_vm.ids

  module_depends_on = [module.kafka_vm, module.kafka_data_disk]
}
...
resource "azurerm_private_dns_a_record" "kafka_dns_zone_a_record" {
  ...
  resource_group_name = data.azurerm_resource_group.sts_kafka_pdns_rg.name
  ttl                 = 360
  records             = [element(module.kafka_nic.ip_addresses, count.index)]
}
